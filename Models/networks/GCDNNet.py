...
# import knn, gcdn_utils, solvers
from utils.netUtils import  *


class GCDNNet(nn.Module):
    ...
    def __init__(self, nic=1, nf=16, iters=3, window_size=32, topK=8, **kwargs):
        super(GCDNNet, self).__init__()
        if nf % 3 != 0:
            raise ValueError(f"Expected nf {nf} to be divisible by 3*circ_rows.")
        gckwargs = {"window_size": window_size, "topK": topK}
        self.INCONV = nn.ModuleList(
            [nn.Conv2d(nic, nf // 3, ks, padding=(ks - 1) // 2, padding_mode='reflect') for ks in [3, 5, 7]])
        ...
        self.wtkargs = (topK, window_size, localMask(window_size, window_size, 3))  # windowedTopK args
        self.iters = iters

    def topK(self, h, *args, **kwargs):
        # if self.eval():
        #	return localTopK(h, *args, **kwargs)
        return windowedTopK(h, *args, **kwargs)

    def forward(self, x, ret_edge=False):
        if ret_edge:
            edge_list = []
        x, params = pre_process(x, self.wtkargs[1])
...
class GClayer(nn.Module):
    def __init__(self, nf, ks=3, window_size=32, topK=8, block_type="LPF", leak=0.2, **kwargs):
        ...
        self.wtkargs = (topK, window_size, localMask(window_size, window_size, ks))  # windowedTopK args
        self.Conv = nn.ModuleList(
            [nn.Conv2d(nf, nf, ks, padding=(ks - 1) // 2, padding_mode='reflect') for _ in range(pre_iters)])
...
class GCDLNet(nn.Module):
    def __init__(self, nic=1, nf=16, ks=7, edge_freq=1, iters=3, window_size=32, topK=8, **kwargs):
        ...
        W = torch.randn(nf, nic, ks, ks);
        W = W / torch.norm(W, dim=(2, 3), keepdim=True)
        adjoint = lambda X: X.transpose(0, 1).flip(2, 3)
        p = (ks - 1) // 2
        conv = lambda x, H: F.conv2d(F.pad(x, (p, p, p, p)), H)
        with torch.no_grad():
            L, _, _ = powerMethod(lambda x: conv(conv(x, W), adjoint(W)),
    ...
    def forward(self, y, ret_edge=False):
        yp, params = pre_process(y, self.wtkargs[1])
        z = ST(self.A[0](yp), self.tau[0])
        for i in range(1, self.iters):
            if ((i - 1) % self.edge_freq) == 0:
                edge = windowedTopK(z, *self.wtkargs) if self.wtkargs[0] is not None else None
